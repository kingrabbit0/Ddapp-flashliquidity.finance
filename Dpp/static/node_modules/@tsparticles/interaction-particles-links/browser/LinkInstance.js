  ...
  constructor(container) {
    ...
    this._drawLinkTriangle = (p1, link1, link2) => {
      ...
      const container = this.container,
        ...
        opacityTriangle =
          triangleOptions.opacity ?? (link1.opacity + link2.opacity) * half;
    ...
    this._drawTriangles = (options, p1, link, p1Links) => {
      ...
      if (
        !(options.links?.triangles.enable && p2.options.links?.triangles.enable)
      ) {
        return;
      }
    ...
    const p1Links = links.filter(
      (l) =>
        ...
        (options.links.frequency >= maxFrequency ||
          this._getLinkFrequency(particle, l.destination) <=
            options.links.frequency)
    );
    for (const link of p1Links) {
      this._drawTriangles(options, particle, link, p1Links);
      if (
        link.opacity > minOpacity &&
        (particle.retina.linksWidth ?? minWidth) > minWidth
      ) {
