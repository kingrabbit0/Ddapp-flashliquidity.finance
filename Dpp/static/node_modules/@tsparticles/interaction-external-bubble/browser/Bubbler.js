...
export class Bubbler extends ExternalInteractorBase {
  constructor(container) {
    ...
    this._clickBubble = () => {
      ...
      for (const particle of query) {
        ...
        const pos = particle.getPosition(),
          ...
          timeSpent =
            (new Date().getTime() -
              (container.interactivity.mouse.clickTime ?? defaultClickTime)) /
            millisecondsToSeconds;
    ...
    this._hoverBubbleOpacity = (particle, ratio, divBubble) => {
      const container = this.container,
        options = container.actualOptions,
        modeOpacity =
          divBubble?.opacity ?? options.interactivity.modes.bubble?.opacity;
      if (!modeOpacity) {
        return;
      }
  ...
  isEnabled(particle) {
    ...
    if (
      !(
        divBubble ||
        (onHover.enable && !!mouse.position) ||
        (onClick.enable && mouse.clickPosition)
      )
    ) {
      return false;
    }
    return (
      isInArray(bubbleMode, onHover.mode) ||
      isInArray(bubbleMode, onClick.mode) ||
      divBubble
    );
  }
