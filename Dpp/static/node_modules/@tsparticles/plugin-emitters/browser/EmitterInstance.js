...
export class EmitterInstance {
  constructor(engine, emitters, container, options, position) {
    ...
    this._prepareToDie = () => {
      ...
      const duration =
          this.options.life?.duration !== undefined
            ? getRangeValue(this.options.life.duration)
            : undefined,
        minDuration = 0,
        minLifeCount = 0;
      if (
    ...
    this._setColorAnimation = (
      ...
      initValue,
      maxValue,
      factor = defaultColorAnimationFactor
    ) => {
    ...
    this._spawnDelay =
      (getRangeValue(this.options.life.delay ?? defaultLifeDelay) *
        millisecondsToSeconds) /
      this.container.retina.reduceFactor;
    ...
    const shapeOptions = this.options.shape,
      shapeGenerator = this._engine.emitterShapeManager?.getShapeGenerator(
        shapeOptions.type
      );
    if (shapeGenerator) {
  ...
  play() {
    ...
    if (this._emitDelay === undefined) {
      ...
      this._emitDelay =
        (delay * millisecondsToSeconds) / this.container.retina.reduceFactor;
    }
  ...
  resize() {
    ...
    this.position =
      initialPosition &&
      isPointInside(initialPosition, this.container.canvas.size, Vector.origin)
        ? initialPosition
        : this._calcPosition();
    this._size = this._calcSize();
    this.size = getSize(this._size, this.container.canvas.size);
