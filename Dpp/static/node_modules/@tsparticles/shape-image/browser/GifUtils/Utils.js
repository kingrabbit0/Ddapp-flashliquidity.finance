...
async function parseImageBlock(
  ...
  const getColor = (index) => {
    const { r, g, b } = (
      localColorTableFlag ? frame.localColorTable : gif.globalColorTable
    )[index];
  ...
  const readBits = (pos, len) => {
    ...
    return (
      ((imageData[bytePos] +
        (imageData[bytePos + 1] << 8) +
        (imageData[bytePos + 2] << 16)) &
        (((1 << len) - 1) << bitPos)) >>>
      bitPos
    );
  };
  if (interlacedFlag) {
    for (
      let code = 0, size = minCodeSize + 1, pos = 0, dic = [[0]], pass = 0;
      pass < 4;
      pass++
    ) {
      if (InterlaceOffsets[pass] < frame.height) {
        ...
        while (!exit) {
          ...
          if (pixelPos === frame.width * 4 * (lineIndex + 1)) {
            lineIndex++;
            if (
              InterlaceOffsets[pass] + InterlaceSteps[pass] * lineIndex >=
              frame.height
            ) {
              exit = true;
            }
...
export async function decodeGIF(gifURL, progressCallback, avgAlpha) {
  ...
  backgroundImage.data.set(
    globalColorTableFlag ? [r, g, b, 255] : [0, 0, 0, 0]
  );
  ...
  try {
    ...
    if (error instanceof EvalError) {
      throw new Error(
        `error while parsing frame ${frameIndex} "${error.message}"`
      );
    }
    throw error;
  }
