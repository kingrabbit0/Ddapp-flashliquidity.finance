...
function rectSideBounce(data) {
  ...
  if (
    (pSide.max >= rectSide.min &&
      pSide.max <= (rectSide.max + rectSide.min) * half &&
      velocity > minVelocity) ||
    (pSide.min <= rectSide.max &&
      pSide.min > (rectSide.max + rectSide.min) * half &&
      velocity < minVelocity)
  ) {
...
export function isInArray(value, array) {
  ...
  return (
    value === array || (isArray(array) && array.indexOf(value) > invalidIndex)
  );
}
...
export function isPointInside(point, size, offset, radius, direction) {
  ...
  return areBoundsInside(
    calculateBounds(point, radius ?? minRadius),
    size,
    offset,
    direction
  );
...
export function deepExtend(destination, ...sources) {
  for (const source of sources) {
    ...
    if (
      sourceIsArray &&
      (isObject(destination) || !destination || !Array.isArray(destination))
    ) {
      destination = [];
    } else if (
      !sourceIsArray &&
...
export function isDivModeEnabled(mode, divs) {
  return !!findItemFromSingleOrMultiple(
    divs,
    (t) => t.enable && isInArray(mode, t.mode)
  );
}
...
export function executeOnSingleOrMultiple(obj, callback) {
  ...
  return isArray(obj)
    ? obj.map((item, index) => callback(item, index))
