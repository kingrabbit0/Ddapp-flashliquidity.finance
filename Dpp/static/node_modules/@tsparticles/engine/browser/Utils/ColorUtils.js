...
export function colorToRgb(input, index, useIndex = true) {
  ...
  const color = isString(input) ? { value: input } : input;
  if (isString(color.value)) {
    return color.value === randomColorValue
      ? getRandomRgbColor()
      : stringToRgb(color.value);
  }
  if (isArray(color.value)) {
    return colorToRgb({
      value: itemFromArray(color.value, index, useIndex),
  ...
  if (max !== min) {
    ...
    res.h =
      r1 === max
        ...
        : (res.h =
            g1 === max
              ? double + (b1 - r1) / (max - min)
              : double * double + (r1 - g1) / (max - min));
  }
  res.l *= lMax;
  res.s *= sMax;
  res.h *= hPhase;
...
export function hslToRgb(hsl) {
  ...
  const half = 0.5,
    ...
    red = Math.min(
      rgbFactor,
      rgbFactor * channel(temp2, temp1, hNormalized + phaseThird)
    ),
...
function setColorAnimation(colorValue, colorAnimation, reduceFactor) {
  ...
  if (colorValue.enable) {
    colorValue.velocity =
      (getRangeValue(colorAnimation.speed) / percentDenominator) * reduceFactor;
    ...
    colorValue.delayTime =
      getRangeValue(colorAnimation.delay) * millisecondsToSeconds;
...
export function updateColorValue(data, range, decrease, delta) {
  ...
  if (
    !data ||
    !data.enable ||
    ((data.maxLoops ?? minLoops) > minLoops &&
      (data.loops ?? minLoops) > (data.maxLoops ?? minLoops))
  ) {
    return;
  }
  if (!data.time) {
  ...
  if (
    (data.delayTime ?? minDelay) > minDelay &&
    data.time < (data.delayTime ?? minDelay)
  ) {
  ...
  const offset = data.offset ? randomInRange(data.offset) : minOffset,
    velocity =
      (data.velocity ?? minVelocity) * delta.factor + offset * velocityFactor,
