...
export class Particles {
  constructor(engine, container) {
    ...
    this._applyDensity = (options, manualCount, group) => {
      ...
      const densityFactor = this._initDensityFactor(numberOptions.density),
        ...
        minLimit = 0,
        optParticlesLimit =
          numberOptions.limit.value > minLimit
            ? numberOptions.limit.value
            : optParticlesNumber,
        particlesNumber =
        ...
        particlesCount = Math.min(
          this.count,
          this.filter((t) => t.group === group).length
        );
      if (group === undefined) {
    ...
    this._initDensityFactor = (densityOptions) => {
      ...
      return (
        ...
        (densityOptions.height * densityOptions.width * pxRatio ** squareExp)
      );
    };
  ...
  addParticle(position, overrideOptions, group, initializer) {
    const limitMode = this._container.actualOptions.particles.number.limit.mode,
      limit =
        group === undefined
          ? this._limit
          : this._groupLimits.get(group) ?? this._limit,
  ...
  update(delta) {
    ...
    this.quadTree = new QuadTree(
      qTreeRectangle(container.canvas.size),
      qTreeCapacity
    );
    for (const [, pathGenerator] of container.pathGenerators) {
