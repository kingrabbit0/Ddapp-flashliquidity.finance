...
export class Particle {
  constructor(engine, container) {
    ...
    this._calculateVelocity = () => {
      ...
      if (
        moveOptions.direction === MoveDirection.inside ||
        moveOptions.direction === MoveDirection.outside
      ) {
        return res;
      }
    ...
    this._checkOverlap = (pos, tryCount = defaultRetryCount) => {
      ...
      if (retries >= minRetries && tryCount > retries) {
        throw new Error(
          `${errorPrefix} particle is overlapping and can't be placed`
        );
      }
      return !!this.container.particles.find(
        (particle) =>
          getDistance(pos, particle.position) < radius + particle.getRadius()
      );
    };
    this._getRollColor = (color) => {
      ...
      const rollFactor = 1,
        ...
        rolled =
          Math.floor(
            ((this.roll.angle ?? none) + backSum) / (Math.PI / backFactor)
          ) % double;
      if (!rolled) {
        return color;
      }
    ...
    this._initPosition = (position) => {
      ...
      this.position = this._calcPosition(
        container,
        position,
        clamp(zIndexValue, minZ, container.zLayers)
      );
      ...
      this.direction = getParticleDirectionAngle(
  ...
  init(id, position, overrideOptions, group) {
    ...
    this.effect = itemFromSingleOrMultiple(
      effectType,
      this.id,
      reduceDuplicates
    );
    ...
    if (overrideOptions) {
      if (overrideOptions.effect?.type) {
        const overrideEffectType = overrideOptions.effect.type,
          effect = itemFromSingleOrMultiple(
            overrideEffectType,
            this.id,
            reduceDuplicates
          );
    ...
    this.effectData = loadEffectData(
      this.effect,
      effectOptions,
      this.id,
      reduceDuplicates
    );
    this.shapeData = loadShapeData(
      this.shape,
      shapeOptions,
      this.id,
      reduceDuplicates
    ...
    this.pathDelay =
      getRangeValue(pathOptions.delay.value) * millisecondsToSeconds;
