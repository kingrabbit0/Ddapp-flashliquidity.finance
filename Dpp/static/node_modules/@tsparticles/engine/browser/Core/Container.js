...
function initDelta(value, fpsLimit = defaultFps, smooth = false) {
  return {
    ...
    factor: smooth
      ? defaultFps / fpsLimit
      : (defaultFps * value) / millisecondsToSeconds,
  };
}
...
export class Container {
    ...
    this._nextFrame = (timestamp) => {
      try {
        if (
          ...
          timestamp <
            this._lastFrameTime + millisecondsToSeconds / this.fpsLimit
        ) {
          this.draw(false);
          return;
        }
      ...
      touchEndHandler = (e) => {
        ...
        if (touched && !touchMoved) {
          const touchEvent = e,
            lengthOffset = 1;
          let lastTouch =
            touchEvent.touches[touchEvent.touches.length - lengthOffset];
          if (!lastTouch) {
            lastTouch =
              touchEvent.changedTouches[
                touchEvent.changedTouches.length - lengthOffset
              ];
            if (!lastTouch) {
              return;
            }
          ...
          const element = this.canvas.element,
            ...
            pos = {
              x:
                lastTouch.clientX -
                (canvasRect ? canvasRect.left : minCoordinate),
              y:
                lastTouch.clientY -
                (canvasRect ? canvasRect.top : minCoordinate),
            };
          clickOrTouchHandler(
            e,
            pos,
  ...
  async export(type, options = {}) {
    ...
    getLogger().error(
      `${errorPrefix} - Export plugin with type ${type} not found`
    );
  }
