...
var isVisibleUncached = function (node, checkParent) {
  return (
    ...
    (!isElementHidden(node) &&
      !isInert(node) &&
      checkParent(getParentNode(node)))
  );
};
export var isVisibleCached = function (visibilityCache, node) {
  ...
  var result = isVisibleUncached(
    node,
    isVisibleCached.bind(undefined, visibilityCache)
  );
  visibilityCache.set(node, result);
  return result;
};
var isAutoFocusAllowedUncached = function (node, checkParent) {
  return node && !isTopNode(node)
    ? isAutoFocusAllowed(node)
      ? checkParent(getParentNode(node))
      : false
    : true;
};
export var isAutoFocusAllowedCached = function (cache, node) {
  ...
  var result = isAutoFocusAllowedUncached(
    node,
    isAutoFocusAllowedCached.bind(undefined, cache)
  );
...
export var notHiddenInput = function (node) {
  return !(
    (isHTMLInputElement(node) || isHTMLButtonElement(node)) &&
...
export var isGuard = function (node) {
  ...
  return Boolean(
    node &&
      ((_a = getDataset(node)) === null || _a === void 0
        ? void 0
        : _a.focusGuard)
  );
};
